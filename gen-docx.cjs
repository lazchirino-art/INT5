...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      ...
      P("Helper compartido por los endpoints de producto CSV: valida la config, descifra la contraseña SMB, detecta el archivo por patrón en cada llamada (soporta nombres dinámicos como data_YYYYMMDD.csv), lo lee y parsea las filas."),
      H("Reintentos de acceso SMB", HeadingLevel.HEADING_2),
      P("El acceso al archivo (detección + lectura) se reintenta ante fallos transitorios de red:"),
      code([
        "Intento 1 -> falla -> espera 10 s ->",
        "Intento 2 -> falla -> espera 10 s ->",
        "Intento 3 -> falla -> ERROR",
      ]),
      B("3 intentos en total, 10 segundos entre cada uno (SMB_MAX_ATTEMPTS / SMB_RETRY_DELAY_MS en server.js)."),
      B("Si cualquier intento tiene éxito, continúa de inmediato."),
      B("Solo se reintenta el acceso SMB. NOT_FOUND se decide en memoria tras leer el archivo y no se reintenta."),
      B("Implicación: con la red caída, la importación puede tardar ~20-30 s en devolver ERROR; producción debe contemplar esa espera."),
